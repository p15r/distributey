...
class TestUnitConfig():
    ...
    def test_get_key_consumer_cert_by_tenant_and_kid(self):
        ...
        pytest.raises(KeyError, config.get_key_consumer_cert_by_tenant_and_kid, 'nonexistingtenant', self.jwe_kid)
    ...
    def test_get_jwt_validation_cert_by_tenant_and_kid(self):
        ...
        pytest.raises(KeyError, config.get_jwt_validation_cert_by_tenant_and_kid, 'nonexistingtenant', self.jwt_kid)

    def test_get_vault_default_role_by_tenant(self):
        cfg = config.get_vault_default_role_by_tenant(self.tenant)
        assert cfg == 'salesforce'

        pytest.raises(KeyError, config.get_vault_default_role_by_tenant, 'nonexistingtenant')

    def test_get_vault_auth_jwt_path_by_tenant(self):
        cfg = config.get_vault_auth_jwt_path_by_tenant(self.tenant)
        assert cfg == 'jwt'

        pytest.raises(KeyError, config.get_vault_auth_jwt_path_by_tenant, 'nonexistingtenant')

    def test_get_vault_transit_path_by_tenant(self):
        cfg = config.get_vault_transit_path_by_tenant(self.tenant)
        assert cfg == 'transit'

        pytest.raises(KeyError, config.get_vault_transit_path_by_tenant, 'nonexistingtenant')
